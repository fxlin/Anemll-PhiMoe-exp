...
# 4/22/25, fxl, based on llama_converter.py. 4/23 took a pass
...
from ..models.phimoe_model import (
    PhimoeModel, 
    PhimoeConfig, 
    PhimoeForCausalLM,
    TEST_DEVICE,
...
class PhimoeConverter(BaseConverter):
    ...
    def GetTransformerStates(model, part=None, prefix="model.model."):
        ...
        if not ENABLE_UNIFIED_CACHE and part:   # fxl: XXX not checked
            assert(False, "impl")  # fxl: XXX not checked            
            # Calculate layer range for this part      fxl: dual, quad, octo.. abtrariy naming... kvcache is chunked as such
            ...
            num_layers_this_part = num_layers *2        # fxl: x2: k & v? 
            print(f"GetTransformerStates part={part} ENABLE_UNIFIED_CACHE={ENABLE_UNIFIED_CACHE} num_layers_this_part={num_layers_this_part} model.config.num_hidden_layers={model.config.num_hidden_layers}")
    ...
    def _get_kv_cache_states(self, model):
        ...
        if hasattr(model.model, "kv_cache_0"):  # Unified cache   fxl: means a cache for all transformer blocks (also "chunks"
        ...
        else:  # Split cache per layer     XXX fxl not checked 
    ...
    def postprocess(self, num_workers=None):
        ...
        if self.converted_model is not None and self.lut_bits is not None:
            print(
                f"Applying LUT quantization with {self.lut_bits} bits and "
                f"{self.per_channel} channels per group using "
                f"{num_workers if num_workers else 1} worker(s)..."
            )
            try:
    ...
    # ------------  below : converstion different "modules" of the model  ------------------- #
    #  input is a complete model (HF). but there's a func for each "module", which extracts that module and only converts that
    #  will create a wrapper for each module, and then convert it to coreml
    def convert_embeddings(self, model):
        ...
        class EmbeddingsWrapper(torch.nn.Module):
            def __init__(self, model):
                super().__init__()
                # "the embedding layer that maps input token IDs to dense vectors"
        ...
        # Create sample input
        #  fxl: bs=1 seq=1. traced model would expect this exact shape. XXX why is this ok?? sufficient for coreml purpose???
        ...
        mlmodel = ct.convert(
            ...
            outputs=[
                ct.TensorType(name="hidden_states", dtype=np.float16)   # fxl: NB embedding in fp16
            ],
            compute_precision=ct.precision.FLOAT16,
            compute_units=ct.ComputeUnit.CPU_AND_NE,
            minimum_deployment_target=ct.target.iOS18,
    ...
    def convert_lm_head(self, model, lut_bits=None, output_dir="."):
        ...
        # fxl: 8way split: cf llama_model.py, an opt that does project using conv2d cf ENABLE_CONV2D
        #  XXX for smaller vocab size, probalby 2way split + conv2d, or just conv2d is enough
        class LMHeadWrapper(torch.nn.Module):
            ...
            def forward(self, hidden_states):   # XXX dindt check thoroughly, assume still correct 
                # fxl: cf LlamaForCausalLM()
                # Reshape input for Conv2d if needed
                if self.split_mode != 'linear':
                    hidden_states = hidden_states.permute(0, 2, 1).unsqueeze(2)
                # fxl: permute and add a dim for conv2d --> (bs, hidden, 1, seq) 

                if self.split_mode == '8way':
                    # fxl: "head" here can be nn.Conv2d() 
        ...
        # Create sample input     fxl: bs=1 seq=1, enough for coreml tracing? same qesutions as embedding
        ...
        # fxl: will output multiple tensor logits? who will commbine & sample across them?  XXX
        ...
        mlmodel = ct.convert(
            traced_model,
            inputs=[         # fxl: bs=1 len=1, b/c only decoding uses lm head 
                ct.TensorType(
                    name="hidden_states",
        ...
        # Apply LUT quantization if specified      fxl: XXX try diff quantizations....
    ...
    def convert_FFN(self, model, chunk_idx=None):
        ...
        class FFNWrapper(torch.nn.Module):      # fxl: make these FFN layers a torch model
            ...
            def __init__(self, model, start_layer=0, end_layer=None):
                ...
                # fxl: part='2' --> unified kvcache
                self.states = PhimoeConverter.GetTransformerStates(model, part='2', prefix="model.model.")
        ...
        try:
            ...
            hidden_states = torch.zeros(     # fxl: still, bs=1 seq=1 
                (1, 1, model.config.hidden_size),  # Shape: (batch, seq_len, hidden)
                dtype=torch.float16,device=TEST_DEVICE
            )
            position_ids = torch.zeros((1,), dtype=torch.long, device=TEST_DEVICE)   # fxl: why need this??
            # fxl: creates a causal attention mask filled with very negative values (like -inf), so that attention logits can be masked before the softmax            
            causal_mask = torch.full(   # fxl: why need this
                (1, 1, 1, self.context_length),  # Shape: (batch, 1, 1, context_len)
                torch.finfo(MODEL_DTYPE).min,
                dtype=MODEL_DTYPE,device=TEST_DEVICE
            )
            current_pos = torch.tensor([0], dtype=torch.long, device=TEST_DEVICE) # fxl: starting point of a subseq (slice
            
            # Trace model  (fxl: a subst of tarnsfomer blocks
            #       fxl: for decoding, would position_ids causal_mask current_pos be needed? XXX
    ...
    def convert_prefill(self, model, chunk_idx=None):
        ...
        class PrefillWrapper(torch.nn.Module):
            def __init__(self, model, start_layer=0, end_layer=None):
                ...
                self.states = PhimoeConverter.GetTransformerStates(model, part='2_prefill', prefix="model.model.")
        ...
        try:
            ...
            # # fxl: here, a slice of input seq (#tokens=bs) ?
            ...
            #   fxl: below, "mask" covers bs (this slice) x context_len 
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Convert Phi MoE model to CoreML format')
    
    # Model configuration
    parser.add_argument('--model', type=str, help='Path to model directory (default: ../Meta-Llama-3.2-1B)')
    parser.add_argument('--prefix', type=str, default='phimoe', help='Prefix for output filenames')
    
    # Conversion options
    parser.add_argument('--batch-size', type=int, default=64, help='Batch size for prefill')   # <-- fxl: a slice of input seq
...
#        only convert specified parts (1,2,3...
#    split part 123.... emb, ffn (ie transformer blocks), lm_head 
...
def test_conversion(model_path=None, output_path=None, context_length=512, lut_bits=4, 
                   ...
                   batch_size=64, num_chunks=1, prefix='phimoe', output_dir='.'):
    """Test conversion of a Phimoe model to ANE format."""
    if model is None:
        ...
        config = PhimoeConfig.from_json(config_path)
    ...
    # fxl: based on specified part ("modules"), convert ...
    #    below logic is messy, whatever...
    ...
    else:       # fxl not converting FFN (transfomer blocks.. (i.e. other parts
    ...
    # Model verification      fxl: just printout input/output names. quite simple
    if converted_model is not None:
        ...
        #   fxl: "forces garbage collection of any internal state"? maybe due to jit trace?
...
# fxl: load config, build torch model (i.e. the whole graph), and load weights (sounds like overkill???
def main():
    args = parse_args()
